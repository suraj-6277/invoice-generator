    ...
    <div className="flex h-[calc(100vh-208px)] justify-center items-center">
      <div>
          ...
          onClick={async () => {
            try {
              ...
              const safeInvoiceDetails = {
                ...
                items: Array.isArray(invoiceDetails.items)
              ...
              const data = await fetch(
                `/flag/1x1/${iconName}.svg`
              );
              const svgFlag = await data.text();
              const countryImageUrl = svgFlag
                ? await svgToDataUri(svgFlag)
                : "";

              if (countryImageUrl && countryImageUrl.length > 0) {
        ...
        <Button
          ...
          type="button"
          className="w-full h-12 rounded-lg text-lg mt-4"
          variant="outline"
        >
